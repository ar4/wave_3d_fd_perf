			...
			for (y = 8; y < ny - 8; y++) {
				for (x = 8; x < nxi; x+=8) {
					...
					for (i = 0; i < 9; i++) {
						...
						__m256 yf2 = _mm256_loadu_ps(&A(f, x - i, y, z));
						yf_xx = _mm256_fmadd_ps(yf1, yfd_coeff, yf_xx);
						yf_xx = _mm256_fmadd_ps(yf2, yfd_coeff, yf_xx);
					}
					for (i = 0; i < 9; i++) {
						__m256 yfd_coeff = _mm256_set1_ps(fd_coeff[i]);
						__m256 yf1 = _mm256_loadu_ps(&A(f, x, y + i, z));
						__m256 yf2 = _mm256_loadu_ps(&A(f, x, y - i, z));
						yf_xx = _mm256_fmadd_ps(yf1, yfd_coeff, yf_xx);
						yf_xx = _mm256_fmadd_ps(yf2, yfd_coeff, yf_xx);
					}
					for (i = 0; i < 9; i++) {
						...
						__m256 yf2 = _mm256_loadu_ps(&A(f, x, y, z - i));
						yf_xx = _mm256_fmadd_ps(yf1, yfd_coeff, yf_xx);
						yf_xx = _mm256_fmadd_ps(yf2, yfd_coeff, yf_xx);
					}
